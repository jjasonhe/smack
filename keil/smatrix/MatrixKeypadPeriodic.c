...
// P5.2 connected to column 2 (keypad pin 3) using 10K pull-up
// P5.1 connected to column 1 (keypad pin 2) using 10K pull-up
// P5.0 connected to column 0 (keypad pin 1) using 10K pull-up
// P4.3 connected to row 3 (keypad pin 7)
// P4.2 connected to row 2 (keypad pin 6)
// P4.1 connected to row 1 (keypad pin 5)
// P4.0 connected to row 0 (keypad pin 4)
...
// Pin 4 -> Row 0 (row starting with 1)
// Pin 5 -> Row 1 (row starting with 4)
// Pin 6 -> Row 2 (row starting with 7)
// Pin 7 -> Row 3 (row starting with *)
...
char MatrixKeypad_Scan(int16_t *Num){
  ...
  pt = &ScanTab[0];
  while(pt->direction){
    P4DIR = pt->direction;         // one output
    P4OUT &= ~0x0F;                // DIRn=0, OUTn=HiZ; DIRn=1, OUTn=0
    for(j=1; j<=10; j++);          // very short delay
    column = (P5IN&0x07);          // read columns
    for(j=0; j<=2; j++){
      if((column&0x01)==0){
        key = pt->keycode[j];
        (*Num)++;
